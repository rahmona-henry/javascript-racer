
//Users enter race track lenght
var lenght = prompt("Hello Racers,please choose your racetrack between: 5 and 15 lenghts " );

if(lenght>5)
{
  alert("Racetrack lenght must be greater than /
   5 lenghts and less than or equal to 15./
   Please choose again");
}
//Check if racetrack must be between 5 and 15 lenghts

//Create racetrack based on users input
function makeTable(){
//Write a for loop to create the number of rows <td> specified in the "row" input field
for 
